...
class Classifier {
 public:
  /** Classifier used to make predictions off a data model. */
  Classifier();
   ...
   */
  size_t ClassifyImage(const std::vector<std::vector<bool>>& board, const DataModel& data_model);
   ...
   */
  void CalculateAccuracy(const std::string& testing_file, const DataModel& data_model);
   ...
   */
  void ReadFileByLine(size_t& count, const DataModel& data_model, const std::string& line, size_t& type_class,
  ...
  std::vector<float> GetCurrentLikelihoodScores() const;
 ...
 private:
  ...
  std::vector<float> curr_likelihood_scores_;
};
} // namespace naivebayes